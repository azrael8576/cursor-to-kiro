...
function locateWorkspaceSkill(
  ...
  for (let index = 0; index < parts.length - 2; index += 1) {
    if (
      (parts[index] === ".cursor" || parts[index] === ".agents") &&
      parts[index + 1] === "skills"
    )
      marker = index;
  }
  if (marker < 0 || parts.at(-1) !== "SKILL.md") return undefined;
  const skillContainer = parts[marker];
  if (!skillContainer) return undefined;
  ...
  return {
    ...
    sourceSkillRoot: path.join(root, ...prefix, skillContainer, "skills"),
...
async function buildSkill(
  ...
  try {
    const bundle = await walkFiles(skillDirectory, {
      excludeDirectory: () => false,
    });
    const symlink = located.symlink || bundle.some(file => file.symlink);
    if (symlink) {
      discoveryConflict =
        "Skill package contains or relies on a symlink; equivalent cross-platform behavior cannot be guaranteed.";
    } else {
      sourceFiles = await Promise.all(
        bundle.map(file =>
          readSourceFile(
            file.absolutePath,
            `${identity.slice(0, -"SKILL.md".length)}${file.identity}`,
          ),
        ),
      );
    }
    const skillSource = sourceFiles.find(
      file => file.absolutePath === located.skillFile,
    );
    const parsed = skillSource
      ? parseMarkdown(decode(skillSource.bytes))
      : { body: "", frontmatter: {}, raw: "" };
    return {
      kind: "skill",
      id: `${scope}:skill:${identity}`,
      identity,
      parsed,
      scope,
      sourceFiles,
...
export async function scanWorkspaceSkills(
  ...
  const files = await walkFiles(root, {
    include: identity => identity.endsWith("/SKILL.md"),
  });
  const located = files
    .map(file =>
      locateWorkspaceSkill(
        root,
        file.absolutePath,
        file.identity,
        file.symlink,
