...
export function convertAnalysis(analysis: Analysis): ManifestEntry[] {
  ...
  switch (analysis.candidate.kind) {
    case "skill": return convertSkill(analysis.candidate);
    case "rule": return convertRule(analysis.candidate);
    case "subagent": return convertSubagent(analysis.candidate);
    case "hook": return convertHook(analysis.candidate);
