...
export function convertSkill(candidate: SkillCandidate): ManifestEntry[] {
  const skillFile = candidate.sourceFiles.find(
    file => path.basename(file.absolutePath) === "SKILL.md",
  );
  if (!skillFile)
    throw new Error(`Skill ${candidate.identity} has no readable SKILL.md`);
  const sourceDirectory = path.dirname(skillFile.absolutePath);
  const destinationDirectory = path.join(
    candidate.destinationSkillRoot,
    candidate.skillName,
  );
