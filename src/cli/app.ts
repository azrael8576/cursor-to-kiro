...
function parseScope(value: string | undefined): MigrationScope | undefined {
  ...
  if (value === "workspace" || value === "user" || value === "both") return value;
  throw new Error(`Invalid --scope value: ${value}`);
}

export async function runCli(argv: string[], io: Pick<Console, "log" | "error"> = console): Promise<number> {
  try {
    const parsed = parseArgs({
      args: argv,
      options: {
        root: { type: "string" }, scope: { type: "string" }, "dry-run": { type: "boolean" },
        yes: { type: "boolean", short: "y" }, version: { type: "boolean" }, help: { type: "boolean" },
      },
      strict: true,
      allowPositionals: false,
    });
    if (parsed.values.help) { io.log(HELP); return 0; }
    if (parsed.values.version) { io.log(VERSION); return 0; }

    const resolved = await resolveWorkspaceRoot(process.cwd(), parsed.values.root);
    ...
    if (!scope) scope = process.stdin.isTTY && process.stdout.isTTY ? await promptScope() : "workspace";
    if (!scope) return 130;
    ...
    if (process.stdin.isTTY && process.stdout.isTTY) {
      ...
      const selectedIds = new Set(selected.filter((analysis) => analysis.selected).map((analysis) => analysis.candidate.id));
    ...
    if (!parsed.values.yes) {
      if (!process.stdin.isTTY || !process.stdout.isTTY) {
        io.error("Non-interactive migration requires --yes. No files were written.");
        return 130;
      }
    ...
    if (reportIssues.length > 0) throw new SafetyError(reportIssues.map((issue) => issue.reason).join(" "));
    const snapshot = snapshotSelectedSources(plan.analyses);
    const result = await commitTransaction([...plan.manifest, output], snapshot);
    io.log(renderTerminalSummary(plan));
    io.log(`Created: ${result.created.length}; already migrated: ${result.alreadyPresent.length}.`);
    io.log("Report: .cursor-to-kiro-report.md");
    return 0;
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    io.error(`cursor-to-kiro: ${message}`);
    return error instanceof SafetyError || (error as { rollbackPerformed?: boolean }).rollbackPerformed ? 2 : 1;
  }
}
