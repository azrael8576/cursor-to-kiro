...
export function analyzeCandidate(candidate: Candidate): Analysis {
  switch (candidate.kind) {
    case "rule": return analyzeRule(candidate);
    case "skill": return analyzeSkill(candidate);
    case "subagent": return analyzeSubagent(candidate);
    case "hook": return analyzeHook(candidate);
    case "agents-md": return analyzeAgentsMd(candidate);
  }
}
