...
export const CURSOR_SUBAGENT_FIELDS = [
  "name",
  "description",
  "model",
  "readonly",
...
export function analyzeSubagent(candidate: SubagentCandidate): Analysis {
  ...
  const unknown = fields.filter(
    field => !CURSOR_SUBAGENT_FIELDS.includes(field as never),
  );
  const explicit = fields.filter(field =>
    ["model", "readonly", "is_background"].includes(field),
  );
  const fieldText = [...unknown, ...explicit].filter(
  ...
  return {
    ...
    cursorBehavior:
      "Cursor subagents inherit the parent model and all local tools by default and have Cursor-specific delegation behavior.",
    kiroGap:
      "Kiro custom subagents use their assigned agent configuration/default model and different tool/resource/permission inheritance.",
    fields,
    selected: false,
  };
}
