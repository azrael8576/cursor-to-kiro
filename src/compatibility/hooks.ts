...
export const HOOK_COMPATIBILITY: readonly HookCompatibility[] = Object.entries(SAME_NAME_CANDIDATES).map(
  ([cursorTrigger, kiroTrigger]) => ({
    cursorTrigger,
    kiroTrigger,
    status: "CONFLICT",
    reason:
...
export function analyzeHook(candidate: HookCandidate): Analysis {
  ...
  return {
    ...
    reason:
      candidate.discoveryConflict ?? candidate.parseError ??
      (possibleTarget
    ...
    cursorBehavior: "Cursor hooks exchange JSON decisions over stdin/stdout and use Cursor-specific exit/fail-open semantics.",
    kiroGap: "Kiro hooks inject stdout/stderr into context and block on different exit and trigger rules.",
    fields,
    selected: false,
  };
}
