...
async function ensureSafeDirectory(
  ...
  while (true) {
    ...
    if (parent === current)
      throw new Error(
        `Cannot find an existing destination ancestor for ${directory}`,
      );
    current = parent;
  }
...
export async function commitTransaction(
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      const stagePath = staged.get(entry.absolutePath);
      if (!stagePath)
        throw new Error(`Missing staged output: ${entry.displayPath}`);
      await copyFile(stagePath, entry.absolutePath, constants.COPYFILE_EXCL);
      created.push(entry.absolutePath);
    }
    for (const entry of entries) {
      const output = await readFile(entry.absolutePath);
      if (!bytesEqual(output, entry.bytes))
        throw new Error(`Post-write validation failed: ${entry.displayPath}`);
    }
    await verifySourceIntegrity(sourceSnapshot);
    return {
      created: created.map(absolute => {
        const entry = entries.find(entry => entry.absolutePath === absolute);
        if (!entry) throw new Error(`Missing created output: ${absolute}`);
    ...
    for (const directory of createdDirectories.reverse()) {
      try {
        ...
        /* Never remove a non-empty/pre-existing directory. */
      }
    }
    throw Object.assign(
      new Error(
        `${error instanceof Error ? error.message : String(error)} Rollback attempted.`,
      ),
      { rollbackPerformed },
    );
  } finally {
