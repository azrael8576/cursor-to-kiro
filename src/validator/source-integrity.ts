...
export function snapshotSelectedSources(
  analyses: Analysis[],
): IntegritySnapshot {
  const files = new Map<string, Uint8Array>();
  for (const analysis of analyses) {
    if (!analysis.selected) continue;
    for (const source of analysis.candidate.sourceFiles)
      files.set(source.absolutePath, source.bytes);
  }
  return { files };
}
...
export async function verifySourceIntegrity(
  ...
  for (const [absolutePath, expected] of snapshot.files) {
    ...
    try {
      ...
      throw new Error(
        `Source integrity check failed for ${absolutePath}: ${String(error)}`,
      );
    }
    if (!bytesEqual(current, expected))
      throw new Error(`Source changed during migration: ${absolutePath}`);
  }
}
