...
export function detectManifestCollisions(entries: ManifestEntry[]): { entries: ManifestEntry[]; issues: DestinationIssue[] } {
  ...
  for (const group of byFolded.values()) {
    ...
    const identical = group.every((entry) =>
      entry.semanticKey === first.semanticKey && bytesEqual(entry.bytes, first.bytes),
    );
    if (identical) {
      deduplicated.push(first);
    } else {
  ...
  const sorted = [...deduplicated].sort((left, right) => folded(left.absolutePath).localeCompare(folded(right.absolutePath)));
  for (let leftIndex = 0; leftIndex < sorted.length; leftIndex += 1) {
    for (let rightIndex = leftIndex + 1; rightIndex < sorted.length; rightIndex += 1) {
      ...
      if (rightPath.startsWith(`${leftPath}${path.sep}`) || leftPath.startsWith(`${rightPath}${path.sep}`)) {
        issues.push({
  ...
  return { entries: deduplicated.filter((entry) => !badIds.has(entry.artifactId)), issues };
}
...
async function existingCaseCollision(absolutePath: string): Promise<string | undefined> {
  ...
  return names.find((name) => name !== basename && name.toLocaleLowerCase("en-US") === basename.toLocaleLowerCase("en-US"));
}

async function unsafeExistingParent(absolutePath: string): Promise<string | undefined> {
  ...
  while (true) {
    ...
    if (parent === current) return `No existing directory ancestor for ${absolutePath}`;
    current = parent;
  }
}

export async function detectExistingDestinationIssues(entries: ManifestEntry[]): Promise<DestinationIssue[]> {
  const issues: DestinationIssue[] = [];
  for (const entry of entries) {
    const unsafeParent = await unsafeExistingParent(entry.absolutePath);
    if (unsafeParent) {
      issues.push({ artifactIds: [entry.artifactId], reason: `Destination has an unsafe file or symlink parent: ${unsafeParent}.` });
      continue;
    }
    const collision = await existingCaseCollision(entry.absolutePath);
    if (collision) {
      issues.push({ artifactIds: [entry.artifactId], reason: `Case-insensitive collision with existing destination ${collision}.` });
      continue;
    }
    const stat = await lstatOptional(entry.absolutePath);
    if (!stat) continue;
    if (stat.isSymbolicLink() || !stat.isFile()) {
      issues.push({ artifactIds: [entry.artifactId], reason: `Destination exists and is not a regular file: ${entry.displayPath}.` });
      continue;
    }
    const existing = await readFile(entry.absolutePath);
    if (!bytesEqual(existing, entry.bytes)) {
      issues.push({ artifactIds: [entry.artifactId], reason: `Destination exists with different content: ${entry.displayPath}.` });
    }
  }
  return issues;
}
