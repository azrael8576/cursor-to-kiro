...
export async function createMigrationPlan(
  ...
  let manifest = analyses
    .filter(analysis => analysis.selected)
    .flatMap(convertAnalysis);
  const collisionResult = detectManifestCollisions(manifest);
  const existingIssues = await detectExistingDestinationIssues(
    collisionResult.entries,
  );
  ...
  manifest = collisionResult.entries.filter(
    entry => !badIds.has(entry.artifactId),
  );
  return {
    analyses,
    manifest,
